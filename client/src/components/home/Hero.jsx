        ...
        <>
            <div className="relative min-h-screen bg-white overflow-hidden">
                ...
                <div className="absolute inset-0 z-0">
                    ...
                    [background-size:24px_24px] [mask-image:radial-gradient(ellipse_50%_50%_at_50%_0%,#000_70%,transparent_100%)]"></div>
                ...
                <nav className="relative z-50 flex items-center justify-between w-full py-6 px-6 md:px-16 lg:px-24 xl:px-40">
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        {!user ? (
                            <>
                                <Link to='/app?state=login' className="px-6 py-2 text-sm font-semibold
                                 text-slate-700 hover:text-indigo-600 transition-colors">
                                    Login
                                </Link>
                                <Link to='/app?state=register' className="px-6 py-2.5 bg-indigo-600
                                 hover:bg-indigo-700 shadow-lg shadow-indigo-200 text-white text-sm font-semibold 
                                 rounded-full transition-all active:scale-95">
                                    Get started
                                </Link>
                            </>
                        ) : (
                            <Link to='/app' className='px-8 py-2.5 bg-slate-900
                             hover:bg-slate-800 text-white text-sm font-semibold rounded-full transition-all shadow-xl'>
                                Dashboard
                            </Link>
                        )}
                    </div>
                ...
                {/* Mobile Menu Sidebar */}
                <div className={`fixed inset-0 z-[60] bg-white transition-transform 
                    duration-500 ${menuOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
                    <div className="flex flex-col h-full p-6">
                        ...
                        <div className="mt-auto flex flex-col gap-4">
                            <Link to="/app" className="w-full py-4 bg-indigo-600
                             text-white text-center rounded-2xl font-bold">Get Started</Link>
                        </div>
                    </div>
                </div>
                ...
                <div className="relative z-10 flex flex-col items-center justify-center pt-20 md:pt-32 px-6 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full
                     bg-indigo-50 border border-indigo-100 text-indigo-700 text-xs font-bold tracking-wide uppercase mb-8">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold max-w-5xl text-slate-900 tracking-tight leading-[1.1]">
                        ...
                        <span className="block mt-2 bg-gradient-to-r from-indigo-600
                         via-violet-600 to-indigo-600 bg-clip-text text-transparent">
                    ...
                    <div className="mt-10 flex flex-col sm:flex-row items-center gap-5">
                        <Link to={'/app'} className="group bg-indigo-600
                         hover:bg-indigo-700 text-white rounded-full px-10 
                         py-4 text-lg font-bold shadow-2xl shadow-indigo-200 transition-all flex items-center gap-2 hover:-translate-y-1">
                            Start Building Now
